...
export function Sidebar() {
  return (
    <aside className="fixed left-0 top-0 h-screen w-56 flex flex-col bg-[#0d0f17] border-r border-[#1e2235] z-40">
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 py-5 border-b border-[#1e2235]">
        ...
        <div>
          <p className="text-sm font-bold text-white leading-none">SIGO</p>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        <p className="px-2 text-[10px] font-semibold uppercase tracking-widest text-slate-600 mb-2">Portfólio</p>

        <NavItem icon={<BarChart3 className="h-4 w-4" />} label="Dashboard" active />

        <p className="px-2 text-[10px] font-semibold uppercase tracking-widest text-slate-600 mt-4 mb-2">Status</p>
      ...
      <div className="px-4 py-4 border-t border-[#1e2235]">
        <p className="text-[10px] text-slate-600">v2.0 · Google Sheets</p>
      </div>
    </aside>
  )
}
...
function StatusItem({ icon, label, color }: { icon: React.ReactNode; label: string; color: string }) {
  return (
    <div className="flex items-center gap-3 px-3 py-1.5 rounded-lg text-slate-500 hover:bg-[#1e2235] cursor-default">
      {icon}
